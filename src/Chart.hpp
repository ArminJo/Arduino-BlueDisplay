...
Chart::Chart() { // @suppress("Class members should be properly initialized")
    ...
    mXTitleText = NULL;
    mYTitleText = NULL;
    XLabelStringFunction = NULL; // required
}
...
void Chart::initXLabelTimestamp(const int aXLabelStartValue, const long aXLabelIncrementValue, const uint8_t aXLabelScaleFactor,
    ...
    mXLabelBaseIncrementValue.TimeValue = aXLabelIncrementValue;
...
void Chart::initYLabelInt(const int aYLabelStartValue, const int aYLabelIncrementValue, const float aYFactor,
    ...
    mYLabelStartValue.LongValue = aYLabelStartValue;
    mYLabelIncrementValue.LongValue = aYLabelIncrementValue;
...
void Chart::drawXAxisAndLabels() {
    ...
    if (mFlags & CHART_X_LABEL_USED) {
        ...
        tIncrementValueForLabel = mXLabelDistance * reduceFloatWithXLabelScaleFactor(mXLabelBaseIncrementValue.FloatValue);
...
void Chart::drawXAxisAndDateLabels(time_t aStartTimestamp,
        int (*aXIntermediateLabelStringFunction)(char *aLabelStringBuffer, time_float_union aXvalue)) {
...
// mXLabelAndGridStartValueOffset is float, so we do not have a overflow here
        tXPixelOffsetOfCurrentLabel = -(mXLabelAndGridStartValueOffset * mGridOrLabelXPixelSpacing)
                / reduceLongWithXLabelScaleFactor(mXLabelBaseIncrementValue.TimeValue); // grid is based on long value
    }
    ...
    long tIncrementValue = mXLabelDistance * reduceLongWithXLabelScaleFactor(mXLabelBaseIncrementValue.TimeValue);
    do {
        ...
        if (tXPixelOffsetOfCurrentLabel >= 0) {
            if ((tGridIndex % tEffectiveXLabelDistance) == 0) {
                 ...
                 */
                tStringLength = aXIntermediateLabelStringFunction(tLabelStringBuffer, tTimeStampForLabel);
...
void Chart::setXLabelStartValueByIndex(const int aNewXStartIndex, const bool doDraw) {
    mXLabelStartValue.FloatValue = mXLabelBaseIncrementValue.FloatValue * aNewXStartIndex;
    if (doDraw) {
        drawXAxisAndLabels();
    }
}
...
// convert to string
// initialize both variables to avoid compiler warnings
        long tValue = mYLabelStartValue.LongValue;
...
#endif
                tValue += mYLabelIncrementValue.LongValue;
            } else {
#if defined(__AVR__)
...
bool Chart::stepYLabelStartValueInt(const bool aDoIncrement, const int aMinValue, const int aMaxValue) {
    bool tRetval = true;
    if (aDoIncrement) {
        mYLabelStartValue.LongValue += mYLabelIncrementValue.LongValue;
        if (mYLabelStartValue.LongValue > aMaxValue) {
            mYLabelStartValue.LongValue = aMaxValue;
            tRetval = false;
        }
    } else {
        mYLabelStartValue.LongValue -= mYLabelIncrementValue.LongValue;
        if (mYLabelStartValue.LongValue < aMinValue) {
            mYLabelStartValue.LongValue = aMinValue;
            tRetval = false;
        }
    }
    drawYAxisAndLabels();
...
// mGridOrLabelYPixelSpacing / mYLabelIncrementValue.LongValue is factor float -> pixel e.g. 40 pixel for 200 value
        tYDisplayFactor = (mYDataFactor * mGridOrLabelYPixelSpacing) / mYLabelIncrementValue.LongValue;
        tYOffset = mYLabelStartValue.LongValue / mYDataFactor;
    } else {
...
// mGridOrLabelYPixelSpacing / mYLabelIncrementValue.LongValue is factor input -> pixel e.g. 40 pixel for 200 value
        tYDisplayFactor = (mYDataFactor * mGridOrLabelYPixelSpacing) / mYLabelIncrementValue.LongValue;
        tYDisplayOffset = mYLabelStartValue.LongValue / mYDataFactor;
    } else {
// Label is float
        tYDisplayFactor = (mYDataFactor * mGridOrLabelYPixelSpacing) / mYLabelIncrementValue.FloatValue;
...
// Label is float
    ...
    BlueDisplay1.drawChartByteBufferScaled(mPositionX, mPositionY, mXDataScaleFactor, -tYDisplayFactor, mAxesSize, aMode,
            mDataColor,
            COLOR16_NO_DELETE, 0, true, aDataPointer, aLengthOfValidData);
...
void Chart::setYLabelStartValue(int yLabelStartValue) {
    mYLabelStartValue.LongValue = yLabelStartValue;
}
...
void Chart::setYLabelBaseIncrementValue(int yLabelBaseIncrementValue) {
    mYLabelIncrementValue.LongValue = yLabelBaseIncrementValue;
}
...
long_float_union Chart::getYLabelStartValue(void) const {
...
// new random data
    ...
    ChartExample.setLabelStringFunction(NULL);
