...
void InitCo2LoggerAndChart() {
     ...
     */
    BlueDisplay1.initCommunication(&signalInitDisplay, NULL); // redraw is registered by startGuiDemo() called by initDisplay()
...
void initializeCO2Array() {
    ...
    if (tCO2ArrayChecksum == sCO2ArrayValuesChecksum) {
        ...
        if (eeprom_read_byte(&sCO2ArrayInEEPROM[sizeof(sCO2ArrayInEEPROM) - 1]) == 0xFF) {
             ...
             */
            Serial.println(F("Clear EEPROM to 0"));
...
#  endif
    ...
    TouchButtonDark.init(&tBDButtonPGMParameterStruct);
    TouchButtonDark.setButtonTextColor(COLOR16_WHITE);
...
// Increment is 1/2 day for scale factor 1
    CO2Chart.initXLabelInteger(0, SECS_PER_DAY / 2, CHART_X_AXIS_SCALE_FACTOR_1, 0);
    CO2Chart.setXLabelDistance(2);

    CO2Chart.initYLabelInt(CO2_BASE_VALUE, CHART_Y_LABEL_INCREMENT, CO2_COMPRESSION_FACTOR, 4); // 200 ppm per grid, 5 for input 1 per 5 ppm , 4 is minimum label string width
...
#endif
    TouchButtonDark.drawButton();
    TouchButtonShowTimeToNextStorage.activate(); // Just listen for touches
}

void drawCO2Chart() {
    drawXAxisTimeDateSettingsStruct tDrawXAxisTimeDateSettings;
...
// For intermediate labels, use midnight as start value

    // Could be done once in init, but then we need RAM instead of stack, because it is needed in the next statement too
    CO2Chart.setXAxisTimeDateSettings(&tDrawXAxisTimeDateSettings, &day, &month, '.', &hour, 5);
...
void printTimeToNextStorage() {
    ...
    uint16_t tSecondsToNext = (sNextStorageMillis - prevMillis) / MILLIS_IN_ONE_SECOND;
